...
#'
#' @export
plot_regional_indicators <- function(data, family) {
  ...
  p <- ggplot(plot_data, aes(x = var_name, y = avg_score)) +
    ...
    labs(
      ...
      subtitle = paste0(family, ", 2020-2024"),
      x        = "Indicator CTF score",
      y        = "CTF average score (0–1)",
      caption  = "Source: CLIAR World Bank."
    ) +
    theme_minimal() +
    facet_grid(~ region) +
    theme(
  ...
  # Save the plot
  ggsave_db(
    here("figures", "indicators", filename)
  )
